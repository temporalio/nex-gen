...
use crate::planning::{
    PlannedField, PlannedOperationResourceFieldBinding, PlannedOperationResourceReturn,
    ...
    PlannedSourcedField, PlannedSpec, PlannedType, PlannedTypeFamily, operation_input_model,
    operation_output_direct_result, relative_descriptor_name,
};
use crate::resources::{
...
use crate::spec::{
    ...
    GeneratedModelSpec, IntSpec, LanguageImportSpec, LanguageImportStyle, LanguageStringSpec,
    OperationSpec, SupportFragmentSpec, TypeReplacementSpec, TypeSpec, VariantSpec,
};
...
fn collect_typescript_language_imports(api_plan: &PlannedSpec) -> Vec<LanguageImportSpec> {
    ...
    for model in api_plan.records.values() {
        ...
        for sourced_field in &model.data.sourced_fields {
...
fn generated_model_type_parameters(
    generated_model: &GeneratedModelSpec<PlannedTypeFamily>,
...
fn typescript_authored_type_annotation(authored_type: &PlannedType) -> String {
    match authored_type {
...
fn resolve_operation<'a>(
    ...
    let (output_proto_ref, output_type_id, output_annotation_default) = match operation
        .output_type()
    {
        Some(
            output @ (PlannedType::External(ExternalTypeSpec::Proto(PlannedProtoType::Message(_)))
            | PlannedType::Record(_)),
        ) => {
            ...
            (
                operation_type_ref(output, api_plan),
                operation_type_id(output, api_plan),
    ...
    let input_full_name = input
        .model_full_name()
        .expect("operation input should be a model");
    let input_model = models.get(input_full_name);
    ...
    Ok(RenderedOperation {
        ...
        return_doc: operation
            ...
            .for_language(Language::TypeScript)
            .map(str::to_string),
        input_proto_ref: operation_type_ref(input, api_plan),
        output_proto_ref,
        input_type_id: operation_type_id(input, api_plan),
        output_type_id,
        input_model_name,
        input_type_parameters,
        input_annotation,
        input_to_proto_expr: input_conversion.to_proto_expr("request"),
        input_nexus_type_id: if matches!(input, PlannedType::Record(_))
            && planned_record_proto(input, api_plan).is_none()
        {
            Some(input_full_name.to_string())
        } else {
            None
        },
...
fn operation_type_ref(model_type: &PlannedType, api_plan: &PlannedSpec) -> String {
    match model_type {
        PlannedType::External(ExternalTypeSpec::Proto(PlannedProtoType::Message(proto))) => {
            message_typescript_interface_ref(&proto.proto)
        }
        PlannedType::Record(record) => planned_record_proto(model_type, api_plan)
            .map(message_typescript_interface_ref)
            .unwrap_or_else(|| record.model_name.clone()),
        _ => panic!("operation type ref should be model-shaped"),
    }
}

fn operation_type_id(model_type: &PlannedType, api_plan: &PlannedSpec) -> String {
    match model_type {
        PlannedType::External(ExternalTypeSpec::Proto(PlannedProtoType::Message(proto))) => {
            proto.proto.full_name.clone()
        }
        PlannedType::Record(record) => planned_record_proto(model_type, api_plan)
            .map(|proto| proto.full_name.clone())
            .unwrap_or_else(|| record.full_name.clone()),
        _ => panic!("operation type id should be model-shaped"),
    }
}

fn planned_record_proto<'a>(
    model_type: &PlannedType,
...
fn resolve_message_value_conversion(
        ...
        return MessageValueConversion {
            ...
            kind: MessageValueConversionKind::Override {
                from_proto: typescript_from_proto_converter(
                    &proto.proto.full_name,
                    language_override,
                ),
                to_proto: typescript_to_proto_converter(&proto.proto.full_name, language_override),
            },
        };
    }
    if let Some(proto) = model_type.proto_message()
        && let Some(authored_type) = &proto.authored_type
    {
        return MessageValueConversion {
            ...
            kind: MessageValueConversionKind::Override {
                from_proto: typescript_default_from_proto_name(&proto.proto.full_name),
                to_proto: typescript_default_to_proto_name(&proto.proto.full_name),
            },
        };
    }
    ...
    let kind = match model_type {
        PlannedType::Record(_) if planned_model.data.proto.is_some() => {
            MessageValueConversionKind::GeneratedModel {
                model_name: planned_model.name.clone(),
            }
        }
...
fn ensure_rendered_enum(planned_enum: &EnumSpec, enums: &mut IndexMap<String, RenderedEnum>) {
    enums
...
fn ensure_rendered_variant(
    ...
    if variants.contains_key(&planned_variant.full_name) {
        return;
    }
...
fn build_flattened_message_field(
    ...
    | PlannedType::Record(_)) = &field.kind
    else {
        return None;
    };
    let full_name = match model_type {
        PlannedType::External(ExternalTypeSpec::Proto(PlannedProtoType::Message(proto))) => {
            &proto.proto.full_name
        }
        PlannedType::Record(record) => &record.full_name,
        _ => unreachable!("field pattern only allows proto and record"),
    };
    ...
    let nested_fields = nested_planned_model.data.fields.clone();
...
fn resolve_planned_value_type(
    ...
    match value_type {
        ...
        PlannedType::Enum(enum_type) => {
            if let Some(planned_enum) = api_plan.enums.get(&enum_type.full_name) {
        ...
        PlannedType::Flags(flags_type) => {
            if let Some(planned_flags) = api_plan.flags.get(&flags_type.full_name) {
        ...
        PlannedType::Variant(variant_type) => {
            if let Some(planned_variant) = api_plan.variants.get(&variant_type.full_name) {
        ...
        PlannedType::Option(inner) => {
            ...
            ResolvedFieldType {
                ...
                message_conversion: inner.message_conversion,
                enum_conversion: inner.enum_conversion,
            }
        }
...
fn required_from_proto_expr(
    ...
    } else if matches!(&field.kind, PlannedType::String) {
        format!("proto.{proto_field_name} === '' ? undefined : proto.{proto_field_name}")
    } else if matches!(&field.kind, PlannedType::Bytes) {
        format!(
...
fn message_typescript_interface_ref(proto: &PlannedProtoTypeInfo) -> String {
...
fn render_model_schema_registrations(
    ...
    for full_name in models.keys() {
        ...
        output.push_str(", {\n");
        output.push_str("  fields: {\n");
        for field in &planned.data.fields {
            output.push_str("    ");
            output.push_str(&typescript_object_key(&typescript_generated_field_name(
                &field.authored_name,
            )));

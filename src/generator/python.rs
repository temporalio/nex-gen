...
fn generate_tree(
    ...
    insert_branch_index_file(&mut files, branch, &model_hoists)?;
...
fn generate_tree_node(
    ...
    match node {
        ...
        ApiSpecNode::Branch(branch) => {
            insert_branch_index_file(files, branch, model_hoists)?;
            for node in branch.children.values() {

...
pub(crate) fn generate(
    ...
    let mut emitted_future_helpers = false;
    for fragment in support_fragments {
        let file_name = support_file_name(fragment)?;
        let mut contents = rewrite_support_package(&fragment.contents, &package.package_name);
        if needs_future_helpers
            && file_name == "model_overrides.go"
            && !contents.contains("type nexGenNexusOperationFuture struct")
        {
            contents = inject_future_support(contents, &package);
        }
        if contents.contains("type nexGenNexusOperationFuture struct") {
            emitted_future_helpers = true;
        }
        files.insert(PathBuf::from(file_name), contents);
    }
    if needs_future_helpers && !emitted_future_helpers {
        files.insert(
            PathBuf::from("model_overrides.go"),
            render_future_support_file(&package),
        );
    }
...
fn inject_future_support(mut contents: String, package: &GoPackageContext) -> String {
    ensure_go_import(&mut contents, "errors");
    if !package.is_self_import("go.temporal.io/sdk/workflow") {
        ensure_go_import(&mut contents, "go.temporal.io/sdk/workflow");
    }
    if !contents.ends_with('\n') {
        contents.push('\n');
    }
    contents.push('\n');
    render_generated_future_helpers(&mut contents, package);
    contents
}

fn ensure_go_import(contents: &mut String, import_path: &str) {
    let quoted_import = format!("\"{import_path}\"");
    if contents.contains(&quoted_import) {
        return;
    }

    if let Some(import_start) = contents.find("import (\n") {
        let insert_at = import_start + "import (\n".len();
        contents.insert_str(insert_at, &format!("\t{quoted_import}\n"));
        return;
    }

    let mut package_line_end = None;
    let mut offset = 0;
    for line in contents.lines() {
        offset += line.len() + 1;
        if line.trim_start().starts_with("package ") {
            package_line_end = Some(offset);
            break;
        }
    }
    if let Some(insert_at) = package_line_end {
        contents.insert_str(insert_at, &format!("\nimport (\n\t{quoted_import}\n)\n"));
    }
}

fn render_future_support_file(package: &GoPackageContext) -> String {
    let mut output = String::new();
    output.push_str("// Code generated by nex-gen. DO NOT EDIT.\n");

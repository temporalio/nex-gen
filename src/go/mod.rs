...
///
/// Walks the [`ApiPlan`] to collect all referenced enums, flags, and models,
/// then renders them into a single `types.go` file inside a directory output.
/// The Go package name is derived from the first service endpoint.
...
/// referenced types (enums, flags, models) have been resolved.
fn ensure_resource_field_types(
    ...
    for field in &resource.fields {
        ensure_resource_field_type(&field.kind, api_plan, enums, flags, models);
    }
    for method in &resource.methods {
        for param in &method.params {
            ensure_resource_field_type(&param.kind, api_plan, enums, flags, models);
        }
    }
}
...
fn ensure_resource_field_type(
    ...
    match kind {
        PlannedFieldKind::Singular(value) | PlannedFieldKind::Repeated(value) => {
            resolve_planned_value_type(value, api_plan, enums, flags, models);
        }
        PlannedFieldKind::Map { key, value } => {
            resolve_planned_value_type(key, api_plan, enums, flags, models);
            resolve_planned_value_type(value, api_plan, enums, flags, models);
        }
    }
}
...
struct ResolvedGoType {
    /// Go type expression (e.g. `"string"`, `"PostalAddress"`, `"[]string"`).
    type_expr: String,
    /// `true` when the resolved type is a struct (message/record) and an
    /// optional field of this type should use a pointer to distinguish the
    /// zero value from absence.
    is_struct: bool,
}
...
fn resolve_operation<'a>(
    ...
    resolve_message_types(&operation.input, api_plan, enums, flags, models);
    if let PlannedOperationOutput::Message(output) = &operation.output {
        resolve_message_types(output, api_plan, enums, flags, models);
    }
...
fn resolve_message_types(
    ...
    ensure_rendered_model(message, api_plan, enums, flags, models);
}
...
/// newly-encountered compound types (enums, flags, models) in the
...
fn resolve_planned_value_type(
    ...
    match value_type {
        PlannedValueType::Scalar(PlannedScalarType::Float) => ResolvedGoType {
            type_expr: "float64".to_string(),
            is_struct: false,
        },
        PlannedValueType::Scalar(PlannedScalarType::Int32) => ResolvedGoType {
            type_expr: "int32".to_string(),
            is_struct: false,
        },
        PlannedValueType::Scalar(PlannedScalarType::Int64) => ResolvedGoType {
            type_expr: "int64".to_string(),
            is_struct: false,
        },
        PlannedValueType::Scalar(PlannedScalarType::Bool) => ResolvedGoType {
            type_expr: "bool".to_string(),
            is_struct: false,
        },
        PlannedValueType::Scalar(PlannedScalarType::String) => ResolvedGoType {
            type_expr: "string".to_string(),
            is_struct: false,
        },
        PlannedValueType::Scalar(PlannedScalarType::Bytes) => ResolvedGoType {
        ...
        PlannedValueType::Variant(_variant_type) => {
            // Variants deferred -- use any as placeholder
            ResolvedGoType {
        ...
        PlannedValueType::Message(message_type) => {
            ...
            ensure_rendered_model(message_type, api_plan, enums, flags, models);
            ResolvedGoType {
        ...
        PlannedValueType::External {
            ...
            if let Some(annotation) = type_name.for_language(Language::Go) {
                ...
                resolve_planned_value_type(fallback, api_plan, enums, flags, models)
            }
        }
        PlannedValueType::Unknown => ResolvedGoType {
            type_expr: "any".to_string(),
            is_struct: false,
        },
    }
...
fn ensure_rendered_model(
    ...
    let fields = planned_model
        .fields
        .iter()
        .map(|field| build_field(field, api_plan, enums, flags, models))
        .collect();
...
/// - Optional scalar/enum/flags fields use the plain type (the zero value
///   represents absence).

...
use crate::planning::{
    PlannedField, PlannedFieldRole, PlannedOperationResourceFieldBinding,
    PlannedOperationResourceReturn, PlannedProtoType, PlannedProtoTypeInfo, PlannedRecordType,
    PlannedResource, PlannedResourceMethod, PlannedResourceMethodBindingSpec,
    PlannedResourceMethodResultKind, PlannedSourcedField, PlannedSpec, PlannedType,
...
use crate::spec::{
    ...
    LanguageImportSpec, LanguageImportStyle, LanguageStringSpec, OperationSpec, RecordSpec,
...
fn collect_python_language_imports(api_plan: &PlannedSpec) -> Vec<LanguageImportSpec> {
    ...
    for record in api_plan.records.values() {
        ...
        for annotation in record.generated_model.field_annotations.values() {
            collect_python_import(annotation, &mut imports);
        }
        for annotation in record.generated_model.field_flattened_annotations.values() {
            collect_python_import(annotation, &mut imports);
        }
        ...
        for sourced_field in &record.data.sourced_fields {
            collect_value_type_imports(&sourced_field.kind, &mut imports);
        }
    }
...
fn operation_service_ref(
    ...
    match model_type {
        PlannedType::External(ExternalTypeSpec::Proto(PlannedProtoType::Message(proto))) => {
            let reference = message_python_ref(&proto.proto)
        ...
        PlannedType::Record(record) => {
            if let Some(proto) = api_plan
                .records
                .get(&record.full_name)
                .and_then(|record| record.data.proto.as_ref())
            {
                let reference = message_python_ref(proto)
                    .expect("descriptor messages should have a Python reference");
                (reference.type_ref, Some(reference.module_path))
            } else {
                (
...
fn build_unpacked_input(
    ...
    for (planned_field, rendered_field) in planned_model.data.fields.iter().zip(model.fields.iter())
    {
...
fn build_flattened_message(
    ...
    let model_type = &planned_field.kind;
...
fn resolve_message_value_conversion(
    ...
    let record = planned_record(api_plan, full_name);
    let kind = match model_type {
        PlannedType::Record(_) if record.data.proto.is_some() => {
            MessageValueConversionKind::GeneratedModel {
                model_name: record.name.clone(),
            }
        }
        PlannedType::Record(_) => MessageValueConversionKind::NativeModel,
        PlannedType::External(ExternalTypeSpec::Proto(PlannedProtoType::Message(proto))) => {
            MessageValueConversionKind::GeneratedModel {
...
fn ensure_rendered_model(
    ...
    let proto = model_type
        .proto_message()
        .map(|message| &message.proto)
        .or(planned_model.data.proto.as_ref());
    let (proto_ref, proto_module_path) = if proto.is_some() {
        (
            Some(
                proto_ref_override
                    .map(str::to_string)
                    .or_else(|| {
                        proto
                            .and_then(|proto| message_python_ref(proto))
                            .map(|reference| reference.type_ref)
                    })
                    .expect("descriptor messages should have a Python reference"),
            ),
            Some(
...
fn build_field(
    ...
    if let PlannedType::Map(key, value) = &field.kind {
...
fn build_sourced_field(
    ...
    if let PlannedType::Map(_, value) = &field.kind {
        let value_type =
            resolve_planned_value_type(value, api_plan, enums, flags, variants, models);
    ...
    match value_type {
        ...
        PlannedType::External(ExternalTypeSpec::Proto(PlannedProtoType::Enum(enum_type))) => {
                ...
                && let Some(type_name) = python_replacement_type_name(replacement)
            {
                ...
                if let Some(reference) = enum_python_ref(&enum_type.proto) {
...
fn required_from_proto_setup_lines(
    ...
    } else if matches!(&field.kind, PlannedType::String | PlannedType::Bytes) {
...
fn message_python_ref(type_info: &PlannedProtoTypeInfo) -> Option<PythonReference> {
    let module_path =
...
fn enum_python_ref(type_info: &PlannedProtoTypeInfo) -> Option<PythonReference> {
    let module_path =
        python_module_path_for_file_name(type_info.file_name.as_deref(), &type_info.package)?;
    let relative_name = relative_descriptor_name(&type_info.full_name, &type_info.package);
    Some(PythonReference {

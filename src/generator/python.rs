...
fn insert_branch_index_file(
    ...
    if exports_json_runtime {
        render_json_public_runtime_import(&mut contents);
        wrote_import = true;
    }
...
impl<'a> ApiPlanner<'a> {
    ...
    fn render_package(
        ...
        let exports_json_runtime = self.model_hoists.is_none()
            && self
                .api_plan
                .external_types()
        ...
        insert_generated_file(
            ...
            if mode == GenerationMode::NativeApi {
                render_package_init(
                    ...
                    self.api_plan,
                    self.model_hoists,
                    exports_json_runtime,
                )
            } else {
                render_definitions_only_package_init(services, &model_names, exports_json_runtime)
            },
        )?;
        insert_generated_file(
            &mut files,
...
fn render_definitions_only_package_init(
    ...
    let service_names = services
        ...
        .collect::<Vec<_>>();
    if exports_json_runtime {
        render_json_public_runtime_import(&mut output);
    }
    ...
    output.push_str("\n__all__ = [\n");
    for name in JSON_PUBLIC_RUNTIME_NAMES
        .iter()
        .copied()
        .filter(|_| exports_json_runtime)
...
fn render_json_public_runtime_import(output: &mut String) {
    let names = JSON_PUBLIC_RUNTIME_NAMES
        .iter()
        .map(|name| (*name).to_string())
        .collect::<Vec<_>>();
    render_named_python_import(output, "._definitions", &names);
}

...
use crate::generator::{GeneratedFiles, ModelWireCapabilities};
...
use crate::spec::{
    ...
    IntSpec, LanguageStringSpec, RecordFieldSpec, RecordSpec, ServiceSpec, SupportFragmentSpec,
...
type PlannedTypeInfo = PlannedProtoTypeInfo;
...
struct ApiPlan {
    ...
    enums: IndexMap<String, PlannedEnum>,
    flags: IndexMap<String, PlannedFlags>,
    variants: IndexMap<String, PlannedVariant>,
    models: IndexMap<String, PlannedModel>,
}
...
struct PlannedOperation {
    ...
    input: PlannedMessageType,
    output: PlannedOperationOutput,
    output_transform: Option<crate::spec::OperationOutputTransformSpec>,
    output_resource_return: Option<PlannedOperationResourceReturn>,
    output_direct_result: bool,
}

#[derive(Debug, Clone)]
enum PlannedOperationOutput {
    Message(PlannedMessageType),
    Resource { type_name: String },
    None,
}

type PlannedOperationResourceReturn = crate::planning::PlannedOperationResourceReturn;
...
struct PlannedResource {
    name: String,
    type_name: String,
    fields: Vec<PlannedResourceField>,
    methods: Vec<PlannedResourceMethod>,
}

#[allow(dead_code)]
#[derive(Debug, Clone)]
struct PlannedResourceField {
    name: String,
    optional: bool,
...
#[derive(Debug, Clone)]
struct PlannedResourceMethod {
    name: String,
    params: Vec<PlannedResourceField>,
    result: Option<PlannedResourceMethodResult>,
    binding: PlannedResourceMethodBindingSpec,
}

#[allow(dead_code)]
#[derive(Debug, Clone)]
struct PlannedResourceMethodResult {
    kind: PlannedResourceMethodResultKind,
    optional: bool,
}

#[allow(dead_code)]
#[derive(Debug, Clone)]
enum PlannedResourceMethodResultKind {
    Resource { type_name: String },
...
#[allow(dead_code)]
#[derive(Debug, Clone)]
enum PlannedResourceMethodBindingSpec {
    Operation {
        operation_name: String,
        request_plan: crate::resources::RequestPlan,
        direct_return: bool,
    },
    Stub,
}
...
struct PlannedModel {
    ...
    sourced_fields: Vec<PlannedSourcedField>,
}
...
struct PlannedField {
    ...
    doc: Option<LanguageStringSpec>,
    annotation_override: Option<LanguageStringSpec>,
    default_value: Option<PlannedFieldDefault>,
    required: bool,
    has_presence: bool,
...
#[allow(dead_code)]
#[derive(Debug, Clone)]
struct PlannedFieldDefault {
    enum_case: String,
}

#[derive(Debug, Clone)]
struct PlannedSourcedField {
    proto_name: String,
    source_expr: String,
    kind: PlannedFieldKind,
}
...
#[derive(Debug, Clone)]
struct PlannedEnumType {
    info: Option<PlannedTypeInfo>,
    name: Option<String>,
    replacement: Option<TypeReplacementSpec>,
}

#[derive(Debug, Clone)]
struct PlannedFlagsType {
    info: PlannedTypeInfo,
    name: String,
}

#[derive(Debug, Clone)]
struct PlannedVariantType {
    info: PlannedTypeInfo,
    name: String,
}

#[derive(Debug, Clone)]
struct PlannedMessageType {
    info: PlannedTypeInfo,
    model_name: String,
    replacement: Option<TypeReplacementSpec>,
    authored_type: Option<PlannedType>,
    source: PlannedMessageSource,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum PlannedMessageSource {
    Proto,
    Wit,
}
...
impl From<ModelWireCapabilities> for ModelCapabilities {
    fn from(capabilities: ModelWireCapabilities) -> Self {
        Self {
...
fn build_go_api_plan(spec: &PlannedSpec) -> Result<ApiPlan> {
    let mut plan = ApiPlan::default();

    for (name, decl) in &spec.types {
        match decl {
            TypeDeclSpec::External(binding) => {
                if let ExternalTypeSpec::Proto(PlannedProtoType::Enum(enumeration)) =
                    &binding.external_type
                    && enumeration.replacement.is_none()
                {
                    plan.enums.insert(
                        enumeration.proto.full_name.clone(),
                        PlannedEnum {
                            info: enumeration.proto.clone(),
            ...
            TypeDeclSpec::Record(record) => {
                let model = planned_model(record, spec);
                plan.models.insert(model.info.full_name.clone(), model);
            }
            TypeDeclSpec::Enum(enumeration) => {
                plan.enums
                    .insert(enumeration.full_name.clone(), planned_enum(enumeration));
            }
            TypeDeclSpec::Flags(flags) => {
                plan.flags
                    .insert(flags.full_name.clone(), planned_flags(flags));
            }
            TypeDeclSpec::Variant(variant) => {
                plan.variants
                    .insert(variant.full_name.clone(), planned_variant(variant, spec));
            }
        }
    ...
    plan.services = spec
        .services
        .iter()
        .map(|service| planned_service(service, spec))
...
fn planned_service(
    ...
    let operations = service
        .operations
        .iter()
        .map(|operation| planned_operation(operation, spec))
        .collect::<Result<Vec<_>>>()?;
    let resources = service
        .resources
        .iter()
        .map(|resource| planned_resource(resource, spec))
        .collect();
...
fn planned_operation(
    operation: &crate::spec::OperationSpec<PlannedTypeFamily>,
    spec: &PlannedSpec,
) -> Result<PlannedOperation> {
    let input = operation
        .input
        .as_ref()
        .and_then(|input| planned_message_type(input, spec))
    ...
    let output = match operation.output.as_ref() {
    ...
    Ok(PlannedOperation {
        ...
        output_resource_return: operation.data.output_resource_return.clone(),
        output_direct_result: operation_output_direct_result(operation),
    })
}

fn planned_message_type(
...
fn planned_model(record: &RecordSpec<PlannedTypeFamily>, spec: &PlannedSpec) -> PlannedModel {
    let fields = record
        .public_fields()
        .map(|(field_name, field)| planned_field(record, field_name, field, spec))
        .collect();
    let sourced_fields = record
        .sourced_fields()
        .map(|(field_name, field, source_expr)| PlannedSourcedField {
    ...
    PlannedModel {
        info: record
            ...
            .proto
            .clone()
            .unwrap_or_else(|| local_type_info(&record.full_name)),
        name: record.name.clone(),
        capabilities: record.data.capabilities.into(),
...
fn planned_field(
    ...
    PlannedField {
        ...
        default_value: field
            .default_value
            .as_ref()
            .map(|default| PlannedFieldDefault {
                enum_case: default.enum_case.clone(),
            }),
        required: field.required,
        has_presence: field.data.has_presence.unwrap_or(!field.required),
        function: field.function.clone(),
        function_args: matches!(role, PlannedFieldRole::FunctionArgs(_)),
        role,
        kind: planned_field_kind(&field.field_type, spec),
    }
}
...
fn planned_resource(
    resource: &crate::spec::ResourceSpec<PlannedTypeFamily>,
    spec: &PlannedSpec,
) -> PlannedResource {
    ...
    PlannedResource {
        ...
        fields: data
            .fields
            .iter()
            .map(|field| planned_resource_field_from_planned(field, spec))
            .collect(),
        methods: data
            .methods
            .iter()
            .map(|method| planned_resource_method_from_planned(method, spec))
            .collect(),
    }
}

fn planned_resource_method_from_planned(
    method: &crate::planning::PlannedResourceMethod,
    spec: &PlannedSpec,
) -> PlannedResourceMethod {
    PlannedResourceMethod {
        ...
        result: method
            .result
            .as_ref()
            .map(|result| planned_resource_result_from_planned(result, spec)),
        binding: match &method.binding {
            crate::planning::PlannedResourceMethodBindingSpec::Operation {
                operation_name,
                request_plan,
                direct_return,
...
fn planned_resource_result_from_planned(
    result: &crate::planning::PlannedResourceMethodResult,
    spec: &PlannedSpec,
) -> PlannedResourceMethodResult {
    let kind = match &result.kind {
        crate::planning::PlannedResourceMethodResultKind::Resource { type_name } => {
            PlannedResourceMethodResultKind::Resource {
                type_name: type_name.clone(),
            }
        }
        crate::planning::PlannedResourceMethodResultKind::Value(kind) => {
            PlannedResourceMethodResultKind::Value(planned_field_kind(kind, spec))
        }
    };
    PlannedResourceMethodResult {
        kind,
...
fn planned_resource_field_from_planned(
    field: &crate::planning::PlannedResourceField,
    spec: &PlannedSpec,
) -> PlannedResourceField {
    PlannedResourceField {
        name: field.name.clone(),
        optional: field.optional,
        kind: planned_field_kind(&field.kind, spec),
        function: field.function.clone(),
    }
}

fn planned_field_kind(field_type: &PlannedType, spec: &PlannedSpec) -> PlannedFieldKind {
...
pub(crate) fn generate(
    ...
    let api_plan = build_go_api_plan(api_plan)?;
    let api_plan = &api_plan;
...
/// as `any`.
fn go_authored_type_annotation(wit_type: &PlannedType) -> String {
    match wit_type {

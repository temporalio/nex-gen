...
pub(crate) fn generate(
    ...
    ApiPlanner::new(api_plan, inline_model_rebuilds)?.build(support_fragments, mode)
}
...
impl<'a> ApiPlanner<'a> {
    fn new(api_plan: &'a PlannedSpec, inline_model_rebuilds: bool) -> Result<Self> {
        Ok(Self {
            api_plan,
            inline_model_rebuilds,
            external_models: PythonExternalModels::new(api_plan)?,
    ...
    fn render_package(
        ...
        insert_generated_file(
            &mut files,
            "service.py",
            render_service_module(services, self.api_plan),
        )?;
...
pub(crate) fn render_tree_model_rebuild_module(
    branch: &ApiSpecBranch<PlannedTypeFamily>,
) -> String {
    let mut modules = Vec::new();
    collect_tree_rebuild_model_modules(branch, &mut modules);
    ...
    if modules.is_empty() {
        ...
        output.push_str("rebuild_models()\n");
        return output;
    }

    let aliases = rebuild_model_aliases(&modules);
    output.push('\n');
        ...
        let names = module
            ...
            .map(|name| {
                format!(
                    ...
                    name,
                    aliases
                        .get(&(module.module_path.clone(), name.clone()))
                        .expect("local rebuild model alias should exist")
                )
            })
            .collect::<Vec<_>>();
    ...
    output.push_str("\n\ndef rebuild_models() -> None:\n");
    for module in &modules {
        let namespace_name = rebuild_namespace_name(&module.module_path);
        output.push_str("    ");
        ...
        for (import_module, names) in &module.module_imports {
            for name in names {
                let Some(alias) = aliases.get(&(import_module.clone(), name.clone())) else {
                    continue;
                };
                output.push_str("        ");
                output.push_str(&python_string_literal(name));
        ...
        for name in &module.model_names {
            ...
            output.push_str("    _ = ");
            output.push_str(alias);
            output.push_str(".model_rebuild(_types_namespace=");
            output.push_str(&namespace_name);
            output.push_str(")\n");
        }
    }
...
#[derive(Debug)]
struct RebuildModelModule {
    module_path: ModulePath,
    model_names: BTreeSet<String>,
    module_imports: BTreeMap<ModulePath, BTreeSet<String>>,
}

fn collect_tree_rebuild_model_modules(
    branch: &ApiSpecBranch<PlannedTypeFamily>,
    modules: &mut Vec<RebuildModelModule>,
) {
        ...
        match child {
            ApiSpecNode::Leaf(leaf) => {
                let model_names = leaf
                    ...
                    .collect::<BTreeSet<_>>();
                if !model_names.is_empty() {
                    modules.push(RebuildModelModule {
                        module_path: leaf.module_path.clone(),
                        model_names,
                        module_imports: leaf.spec.data.module_imports.clone(),
                    });
                }
            }
            ApiSpecNode::Branch(branch) => collect_tree_rebuild_model_modules(branch, modules),
        }
    }
}

fn rebuild_model_aliases(modules: &[RebuildModelModule]) -> BTreeMap<(ModulePath, String), String> {
    modules
        .iter()
        .flat_map(|module| {
            module.model_names.iter().map(|name| {
                (
                    (module.module_path.clone(), name.clone()),
                    rebuild_model_alias(&module.module_path, name),
                )
            })
        })
        .collect()
...
fn rebuild_model_alias(module_path: &ModulePath, name: &str) -> String {
    let mut parts = module_path
        .0
        .iter()
        .map(|segment| python_alias_part(segment))
        .collect::<Vec<_>>();
    parts.push(python_alias_part(name));
    format!("_{}", parts.join("_"))
}

fn rebuild_namespace_name(module_path: &ModulePath) -> String {
    let mut parts = module_path
        .0
        .iter()
        .map(|segment| python_alias_part(segment))
...
fn python_alias_part(value: &str) -> String {
    let mut out = value
        .chars()
        .map(|character| {
...
fn render_models_module(
    ...
    render_generated_file_header(&mut output);
    output.push('\n');
    let import_scan_body = if api_plan.data.module_imports.is_empty() {
        body.clone()
    } else {
        format!("{body}\ntyping.TYPE_CHECKING\n")
    };
    ...
    let wrote_module_model_imports = render_python_module_model_type_checking_imports(
        &mut output,
        api_plan,
...
fn render_service_module(services: &[RenderedService<'_>], api_plan: &PlannedSpec) -> String {
    let mut module_imports = BTreeSet::new();
...
fn render_model_name_imports(
    ...
    let mut foreign_names = BTreeMap::<ModulePath, Vec<String>>::new();
    for name in names {
    ...
    render_named_python_import(output, local_module, &local_names);
    for (module_path, names) in foreign_names {
        render_named_python_import(
            output,
...
fn module_common_prefix_len(left: &[String], right: &[String]) -> usize {
    left.iter()
        .zip(right.iter())
...
fn render_package_init(
    ...
    if !used_runtime_model_names.is_empty() {
        render_named_python_import(&mut output, ".models", &used_runtime_model_names);
    }
        ...
        .flat_map(|service| {
            service.operations.iter().flat_map(|operation| {
                let input_model = operation
                    .input
                    .as_ref()
                    .and_then(|input| input.type_ref.strip_prefix("models."))
                    .map(str::to_string);
                let output_model = (!resource_names.contains(&operation.output_ref))
                    .then(|| {
                        operation
                            .output_ref
                            .strip_prefix("models.")
                            .unwrap_or(&operation.output_ref)
                            .to_string()
                    })
                    .filter(|name| name != "None" && !name.contains('.'));

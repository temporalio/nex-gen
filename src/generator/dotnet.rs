...
use crate::planning::{
    PlannedField, PlannedOperationResourceFieldBinding, PlannedProtoType, PlannedProtoTypeInfo,
    PlannedResource, PlannedResourceMethod, PlannedResourceMethodBindingSpec,
    PlannedResourceMethodResultKind, PlannedSpec, PlannedType, PlannedTypeFamily,
    operation_input_model,
};
use crate::resources::{RequestPlan, RequestPlanSource, ResolvedResourceBindingSource};
use crate::spec::{
    EnumSpec, ExternalTypeSpec, FlagsSpec, FunctionFieldSpec, IntSpec, OperationSpec, RecordSpec,
    ServiceSpec, SupportFragmentSpec, TypeReplacementSpec, TypeSpec, VariantSpec,
};
...
fn operation_has_flattened_convenience(
    ...
    !model.data.fields.is_empty() && model_function_fields_flattenable(model)
}
...
fn flattened_method_parameter_docs(
    ...
    for field in &model.data.fields {
        ...
        if let Some(nested_model) = flattened_nested_model(field, api_plan) {
            for nested_field in &nested_model.data.fields {
...
fn render_flattened_generic_parameters(
    ...
    if model.data.fields.iter().any(|field| {
        matches!(
            overload.function_mode(field),
            FlattenedFunctionMode::Expression
        ) && field
...
fn model_has_options_fields(model: &PlannedModel, api_plan: &PlannedSpec) -> bool {
    model.data.fields.iter().any(|field| {
        if let Some(nested_model) = flattened_nested_model(field, api_plan) {
            nested_model.data.fields.iter().any(field_is_options_field)
        } else {
...
fn model_options_required(model: &PlannedModel, api_plan: &PlannedSpec) -> bool {
    model.data.fields.iter().any(|field| {
        if let Some(nested_model) = flattened_nested_model(field, api_plan) {
            field.required
                && nested_model.data.fields.iter().any(|nested_field| {
...
fn collect_public_field_kind_models(
    ...
    match kind {
        PlannedType::List(value) => collect_public_value_models(names, value, api_plan),
        PlannedType::Map(key, value) => {
...
fn resource_method_operation_call_args(
    ...
    if operation_has_flattened_convenience(operation, model, api_plan) {
        if model
            ...
            .fields
            .iter()
            .any(|field| field.function.is_some())
        {
...
fn resource_method_request_model_init_expr(
    ...
    for field in fields {
        let model_field = model.data.fields.iter().find(|candidate| {
...
fn dotnet_to_proto_converter(model_type: &PlannedType) -> Option<&str> {
    model_type
        .proto_message()?
        .replacement
        .as_ref()
...
fn field_type(field: &PlannedField, api_plan: &PlannedSpec) -> String {
...
fn model_field_type(model: &PlannedModel, field: &PlannedField, api_plan: &PlannedSpec) -> String {
...
fn function_args_parameter_type(model: &PlannedModel, field: &PlannedField) -> Option<String> {
    if let Some(field_type) = model.generated_model.field_type(&field.proto_name)
...
fn function_args_proto_authored_type(field: &PlannedField) -> Option<&PlannedType> {
    match &field.kind {
...
fn dotnet_message_type(model_type: &PlannedType) -> String {
...
fn dotnet_proto_type_name_for_message(model_type: &PlannedType) -> String {
